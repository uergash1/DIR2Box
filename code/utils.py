...
def parse_args():
    ...
    parser.add_argument("--save_model", default=0, type=int, help='save model or not')
...
def bpr_loss(query_point, pos_center, pos_offset, neg_center, neg_offset, gamma):
    pos_distance = vector_box_distance(query_point, pos_center, pos_offset, gamma)
    neg_distance = vector_box_distance(query_point, neg_center, neg_offset, gamma)
    loss = - torch.log(torch.sigmoid(neg_distance - pos_distance))
    return loss.mean()

def hinge_loss(query_point, pos_center, pos_offset, neg_center, neg_offset, delta, gamma):
    pos_distance = vector_box_distance(query_point, pos_center, pos_offset, gamma)
    neg_distance = vector_box_distance(query_point, neg_center, neg_offset, gamma)
    loss = torch.relu(pos_distance - neg_distance + delta)
    return loss.mean()

def ndcg_eval(model, data, current_fold, epoch, mode, args, device, config, write_result=False):
    ndcg_results = {}
    np_results = {}
    model.eval()

    with torch.no_grad():
        query_embeddings, query_ids, document_embeddings, y_true = data.get_eval_data(current_fold, mode)
        query_embeddings = query_embeddings.to(device)
        document_embeddings = document_embeddings.to(device)

        center, offset = model(document_embeddings)
        query_embeddings = model(query_embeddings, query=True)
        ...
        for query_embedding in tqdm(query_embeddings):
            ...
            for resource_id in range(center.shape[0]):
                dist = vector_box_distance(query_embedding, center[resource_id], offset[resource_id])
        ...
        if write_result:
            with open(f'ranks/{args.llm}/{config}_epoch{epoch}_fold{current_fold}.txt', 'w') as f:
                for i, query_id in enumerate(query_ids):
                    resource_rank = sorted(range(len(y_score[i])), key=lambda j: y_score[i][j], reverse=True)
                    resource_rank = ' '.join([str(r) for r in resource_rank])
                    f.write(f'{query_id} {resource_rank}\n')
